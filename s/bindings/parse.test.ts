...
import {runTheseTestsOnEachBindType} from "./parsing/run-these-tests-on-each-bind-type.js"

export default <Suite>{

	"action binds": runTheseTestsOnEachBindType(channel => ({

		"bind with no content": async() => {
			const emoji = emojis[channel]
			const binds = <Binds>parseBindings(`
				${emoji} Benevolent :: `)[channel]
			expect(Object.keys(binds).length).equals(1) },

		"single bind": async() => {
			const emoji = emojis[channel]
			const binds = <Binds>parseBindings(`
				${emoji} Benevolent :: Alpha `)[channel]
			expect(Object.keys(binds).length).equals(1)
		...
		"two binds": async() => {
			const emoji = emojis[channel]
			const binds = <Binds>parseBindings(`
				${emoji} Benevolent :: Alpha Bravo `)[channel]
